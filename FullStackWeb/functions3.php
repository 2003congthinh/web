...
function nameCheck($fullname) {
    ...
    if (preg_match($textPattern, $fullname)) {
        $result = true;
    }
    else {
        $result = false;
...
function createAccount($username, $password, $fullname, $address, $img_file) {
    ...
    file_put_contents('accounts.txt', 'Fullname' .  $username . ': '  . $fullname ."\n", LOCK_EX | FILE_APPEND);
...
function passwordLogin($username, $password) {
    ...
    $checkUser ="/(?<=Username: $username Password: )(.*?)(?= Fullname)/";
